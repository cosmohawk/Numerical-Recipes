/* ch. 7.2 Completely hashing a large array */

void psdes(Uint &lword, Uint &rword) {
  /* Pdeudo-DES hashing of the 64-bit word (lword,rword).
  ...
  for (i=0; i<NITER; i++) {
    ...
    /* The bit-rich constants c1 and c2 guarantee lots of
       non-linear mixing. */
    ia = (iswap=rword) ^ c1[i];
    itmpl = ia & 0xffff;
    itmph = ia >> 16;
...
void hashall(VecUint &arr) {
  /* Replace the array arr with the same sized hash, all of
     whose bits depend on all of the bits in arr.
